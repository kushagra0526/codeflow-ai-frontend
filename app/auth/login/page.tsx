...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsAnalyzing(true)
      await login(formData.leetcode_username, '')
      // Analysis happens in the login function
      router.push("/dashboard")
    } catch (err: any) {
      setError(err.message || "Login failed. Please check your LeetCode username.")
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    We'll fetch your LeetCode profile data automatically
                  </p>
                </div>
